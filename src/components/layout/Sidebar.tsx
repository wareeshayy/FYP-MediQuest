...
export function Sidebar({ className }: SidebarProps) {
  ...
  const getFilteredNavItems = () => {
    if (role === "educator") {
      // Experts: Remove Dashboard and Create Quiz, keep only Analytics and Expert
      return navItems.filter(item => 
        item.href !== "/dashboard" && 
        item.href !== "/create-quiz"
      );
    } else if (role === "student") {
      // Students: Remove Expert link
      return navItems.filter(item => item.href !== "/educator");
    }
      ...
      <div className="flex h-full flex-col relative z-10">
        ...
        <div className="flex h-20 items-center justify-center border-b border-border/40 px-6 backdrop-blur-sm bg-white/10 dark:bg-slate-900/10">
          <Link
            ...
            className="flex items-center space-x-4 group transition-all"
          >
            <motion.div
              whileHover={{ scale: 1.1, rotate: 5 }}
              className="relative"
            >
              <div className="absolute inset-0 bg-gradient-to-br from-teal-400/40 via-cyan-400/20 to-blue-400/40 rounded-2xl blur-xl group-hover:blur-2xl transition-all" />
              <div className="relative bg-gradient-to-br from-teal-500 via-cyan-500 to-blue-600 p-3 rounded-xl shadow-lg">
